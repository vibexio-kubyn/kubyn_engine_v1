...
def run_engine3(user_id: int) -> dict:
    """
    Runs Engine 3 projections and AI explanation for a user.
    Depends on Engine 1 confidence/personality data and Engine 2 goals.
    """
    ...
    avg_monthly_expense = float(
        expense_row["avg_monthly_expense"] if expense_row else 0.0
    )
    ...
    monthly_income = float(
        income_row["monthly_income"] if income_row else 0.0
    )
    ...
    projection_years_consistent = len(consistent) // 12
    projection_years_inconsistent = len(inconsistent) // 12
    ...
    explanation = build_explanation(
        ...
        engine1["archetype"],
        projection_years_consistent,
        projection_years_inconsistent
    )
    ...
    execute("""
        INSERT INTO engine3_scores (
            ...
            consistent_goal_year,
            inconsistent_goal_year,
            ai_explanation
        )
    ...
    return {
        ...
        "goal_timeline": {
            "consistent_years": projection_years_consistent,
            "inconsistent_years": projection_years_inconsistent
        },
        "constraints": {
